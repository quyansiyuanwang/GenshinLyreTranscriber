...
`--auto` 使用确定性分位数检测：时长使用 15% 分位、confidence 使用 30% 分位、音高使用
30% 分位，并限制在安全范围。推荐规则为“时长达到下限”或“confidence 与音高同时达到
下限”，仍是标准 FilterSpec v1，可在 TUI 中继续编辑。`balanced` 和 `melody` 是固定
预设；所有预设都保留原结果并生成新的筛选版本。
...
进程树清理。结果页读取正式 `report.json` 展示计数、告警和产物，并提供播放/暂停、停止、
音量调整与失败后参数页重试；没有试听文件或设备不可用时显示明确降级状态。
