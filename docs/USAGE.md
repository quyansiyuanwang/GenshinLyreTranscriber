...
结果页底部提供固定的 A/B 同轨对比：`A` 为原始素材，`B` 为映射后的 `preview.wav`。
切换来源时会读取当前播放位置并在新来源恢复，不会叠加第二路播放。FLAC、MP3、MP4 等原音会
通过随包 FFmpeg 按需解码到本地 WAV 缓存；若没有原始素材或 `preview.wav`，对应按钮会明确
禁用，不会用其他来源冒充。
