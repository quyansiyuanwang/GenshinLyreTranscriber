...
#[test]
fn performance_command_accepts_a_result_directory() {
    let root = unique_test_dir("performance-command");
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).unwrap();
    let source = root.join("result");
    std::fs::create_dir_all(&source).unwrap();
    let output_dir = root.join("performance-revision");
    let output = Command::new(binary())
        .args(["performance"])
        .arg(&source)
        .arg("--output")
